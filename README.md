...
- An [npmjs.com](https://www.npmjs.com) account with access to the `@opendevstack` scope.
- You must be logged in to the npm registry: `npm login`

### Steps

#### 1. Build the package

The `dist/` and `loader/` directories (declared in the `files` field of `package.json`) are generated by the build step and must be up to date before publishing.
...
Update the `version` field in `package.json` following [Semantic Versioning](https://semver.org/), then record the change in `CHANGELOG.md`.
...
The package is scoped to `@opendevstack`, so you need to explicitly opt in to publishing it as a public package:
...
Only the contents of the `files` array in `package.json` are included in the tarball:

| Directory | Contents |
|-----------|----------|
| `dist/` | Compiled component bundles, type declarations, and collection manifest |
| `loader/` | Custom Elements loader for lazy-loading in Angular/React apps |

You can preview the exact file list without publishing by running:
